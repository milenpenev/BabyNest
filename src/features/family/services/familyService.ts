...
export const familyService = {
  ...
  async updateFamilyName(
    ...
    if (!data) {
      throw new Error("Family name was not updated.");
    }
  ...
  async createAuditEntry(input: {
    ...
    if (!row) {
      throw new Error("Audit entry was not created.");
    }
  ...
  async listAuditEntries(familyId: string): Promise<Array<{
    id: string;
    familyId: string;
    actorProfileId: string;
    action: string;
    ...
    const { data, error } = await supabase.rpc(
      "list_family_audit_entries",
      {
        requested_family_id: normalizedFamilyId,
        requested_limit: 100,
      },
    );
    ...
    if (error) {
      console.error(
        "[BabyNest audit] Failed to load persistent audit",
        {
